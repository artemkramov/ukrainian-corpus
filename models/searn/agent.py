...
class Agent:
    ...
    def form_training_set(self, policy, actions, policy_reference, metric):
        ...
        for state in self.states[:-1]:
            ...
            for action in actions:
                ...
                # print(losses[-1], state_end.clusters, self.state_gold.clusters, action)

            # losses = list(reversed(losses))
            # Append state with corresponding losses to the training set

            cluster1, cluster2 = Agent.get_matrices(policy, state, self.mentions)
    ...
    def __init__(self, _tokens, start=True):
        self.states = []
            ...
            for idx, token in enumerate(mention.tokens):
                if mention.is_entity:
                    ...
                    if not (group in clusters):
                        clusters[group] = []
                    if idx == 0:
                        clusters[group].append([t.WordOrder for t in mention.tokens])
                words.append(vars(token))

        return {"tokens": words, "clusters": clusters}

    def state_to_list(self, state, offset=0):
